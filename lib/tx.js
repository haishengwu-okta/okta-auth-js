 ...
 */
// @flow
import type { HttpRequestOption, SystemConfig, OktaAuth, ApiLink } from './types';
...
var config /*: SystemConfig */            = require('./config');
...
function transactionStatus(sdk /*: OktaAuth */, args /*: {} */) {
  args = addStateToken(sdk, args);
  return http.post(sdk, sdk.options.url + '/api/v1/authn', args);
}

function resumeTransaction(sdk /*: OktaAuth */, args /*: {stateToken?: string} */) {
...
function transactionExists(sdk /*: OktaAuth */) {
  // We have a cookie state token
  return !!sdk.tx.exists._getCookie(config.STATE_TOKEN_COOKIE_NAME);
}

function postToTransaction(sdk /*: OktaAuth */, url /*: string */, args /*: ?{} */, options /*: ?HttpRequestOption */) {
      ...
      return pollFn()
        .then(function (pollRes) {
          ...
          if (pollRes.factorResult && pollRes.factorResult === 'WAITING') {
            ...
            // Continue poll
            return Q.delay(delay || config.DEFAULT_POLLING_DELAY)
              .then(recursivePoll);
...
/*::
type Link2FnResultA = (name: string, opts: {}) => any;
type Link2FnResultB = () => any;
type Link2FnResultC = (opts: {}) => any;
type Link2FnResult = Link2FnResultA | Link2FnResultB | Link2FnResultC | void
*/
function link2fn(sdk, res, obj, link /*: Array<ApiLink> | ApiLink */, ref) /*: Link2FnResult */ {
  if (Array.isArray(link)) {
    return function(name /*: string */, opts) {
      if (!name) {
