 ...
 */
// @flow
import type { OktaAuth, OAuthOptions, AuthConfig, ValidateClaimOption, Claim, TokenObject } from './types';
...
function isToken(obj /*: TokenObject */) {
  if (obj &&
...
function addListener(eventTarget /*: EventTarget */, name /*: string */, fn /*: EventListener */) {
...
function removeListener(eventTarget /*: EventTarget */, name /*: string */, fn /*: EventListener */) {
...
function loadFrame(src /*: string */) {
  ...
  return document.body && document.body.appendChild(iframe);
}

function loadPopup(src /*: string */, options /*: {popupTitle?: string} */) {
...
function getWellKnown(sdk /*: OktaAuth */, issuer /*: string */) {
  return http.get(sdk, (issuer || sdk.options.url) + '/.well-known/openid-configuration', {
    cacheResponse: true
  });
}

function getKey(sdk /*: OktaAuth */, issuer /*: string */, kid /*: string */) {
  return getWellKnown(sdk, issuer)
  .then(function(wellKnown /*: {jwks_uri: string} */) {
...
function validateClaims(sdk /*: OktaAuth */, claims /*: Claim */, validationParams /*: ValidateClaimOption */) {
...
function getOAuthUrls(sdk /*: OktaAuth */, oauthParams /*: OAuthOptions */, options /*: AuthConfig */) {
  options = options || {};
