 ...
 */
// @flow
import type { OktaAuth, HttpRequestOption, ServerError } from './types';
...
function httpRequest(sdk /*: OktaAuth */, options /*: HttpRequestOption */) {
  options = options || {};
  var url = options.url,
  ...
  var headers /*: {Authorization?: string } */ = {
  ...
  return new Q(sdk.options.ajaxRequest(method, url, ajaxOptions))
    ...
    .fail(function(resp) {
      ...
      if (util.isString(resp.responseText)) {
        try {
...
function get(sdk /*: OktaAuth */, url /*: string */, options /*: HttpRequestOption */) {
...
function post(sdk /*: OktaAuth */, url /*: string */, args /*: ?{} */, options /*: ?HttpRequestOption */) {
  url = util.isAbsoluteUrl(url) ? url : sdk.options.url + url;
  var postOptions = {
    url: url,
    method: 'POST',
